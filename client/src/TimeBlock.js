...
export class TimeBlock extends Component {
  ...
  drawChart() {
    ...
    var m = { top: 40, right: 20, bottom: 20, left: 20 }, width = 360, barSize = 20, height = ((dRange[1] - dRange[0]) / (24 * 60 * 60 * 1000)) * barSize;
    var svg = d3.select('#' + this.props.id).append("div")
    ...
    var x = d3.scaleTime().domain([0, 14]).range([0, width]);
    var y = d3.scaleTime().domain(dRange).range([0, height]);
    ...
    svg.append("g")
      ...
      .attr("x", function (d) {
        var startDate = new Date(d.start), hr = startDate.getHours(), mn = startDate.getMinutes(), dur = hr - 10 + (mn / 60); // -10 because we're rendering 0..14 hours of the day?
        return x(dur);
      })
      .attr("y", function (d) {
      ...
      .attr("width", function (d) {
        var hstart = new Date(d.start), hstop = new Date(d.stop);
      ...
      .datum(function (d) { return Date.parse(d.start); });
    /*add axes and grid*/
    var xAxis = d3.axisTop(tfh)
      .ticks(6).tickFormat(d3.timeFormat("%I %p"));
    var xGrid = d3.axisBottom(tfh)
      .ticks(6);
  ...
  render() {
    return <TimeBlockDiv id={this.props.id}></TimeBlockDiv>;
  }
}
